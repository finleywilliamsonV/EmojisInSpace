package  {
	...
	*/
	...
	public class LaserBall_v3Cluster_Level_3 extends Sprite implements IPoolable, IGameObject {
		...
		public static var _shotRate : int = 12;
		
		
		// Constructor
		public function LaserBall_v3Cluster_Level_3() {
		...
		public function addExtraLasers() : void {
			...
			var tempRef;
			var tempRandomSpeed : Number = 16 + int(Math.random() * 40)/10;
			tempRandomSpeed /= 2;
			...
			for (var i : int = -3; i < 4; i ++) {
				...
				if (i == -1 || i == 1) {
					tempRef.setSpeed(i * 3, tempRandomSpeed * 1.25);
					tempRef.scaleX = .8;
					tempRef.scaleY = .8;
				} else if (i == -2 || i == 2) {
					...
					tempRef.setSpeed(i * 5, tempRandomSpeed * .75);
					tempRef.scaleX = .4;
					tempRef.scaleY = .4;
				} 
